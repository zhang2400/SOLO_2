...
class VisionRobot:
    def __init__(self):
        ...
        self.DISTANCE_TO_MOVE_TIME = 70 # 距离与前进时间的比例系数
            ...
            if abs(error) <= self.ALIGN_THRESHOLD:
                ...
                if move_after_align:
                    ...
                    if distance > self.TARGET_DISTANCE_CM:
                        # 计算前进时间(简单线性关系)
                        move_time = int((distance - self.TARGET_DISTANCE_CM) * self.DISTANCE_TO_MOVE_TIME)
                        move_time = max(100, min(move_time, 2000))  # 限制在100-2000ms之间
                        print(f"前进 {move_time}ms")
                        self.mv.move_forward(15, move_time)
    ...
    def process_align_tank(self, results):
        ...
        time.sleep(0.5)
        if a > 0:
            self.mv.move_forward(25, 1180)
            time.sleep(2)
        else:
            self.mv.move_forward(25, 980)
            time.sleep(2)
    ...
    def process_align_zuifan(self, results):
        ...
        time.sleep(0.5)
        if b > 0:
            self.mv.move_forward(25, 1380)
            time.sleep(2)
        else:
            self.mv.move_forward(25, 1035)
            time.sleep(2)
    ...
    def xunxian(self):
        ...
        while True:
            ...
            line_pid = PID(Kp=-0.3, Kd=0, outmax=400, outmin=-400)
            pid_out = line_pid.Calc(diff, 0)
            print(diff)
            self.mv.any_ward(speed=18, turn=pid_out, times=50)

            if elapsed_time >= 6.5 and elapsed_time < 7.5:
                self.mv.stop()
                time.sleep(2)
                self.align_and_move_to_cross(offset=-45)  # 最后阶段使用特定偏移
                time.sleep(1)
                self.mv.move_forward(25, 1570)
                time.sleep(2)
                self.mv.stop()
                break
